...
> **CNI mode note:** This page describes addon scheduling behavior in the default `vpc-cni` mode. For the optional EC2-only Cilium overlay mode, see [CNI_MODES.md](./CNI_MODES.md). In `cilium-overlay` mode, remove `vpc-cni` from `eks_addons`; the base layer bootstraps Cilium before EC2 managed node groups.
