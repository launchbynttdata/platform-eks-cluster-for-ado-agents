...
@test "multi-platform structure test path builds temporary manifest once then promotes it" {
  run "$SCRIPT" \
    -r ado-agent \
    -t v1.2.3 \
    --context "$CTX" \
  ...
  grep -q "docker buildx build .* --platform linux/amd64,linux/arm64 .* -t 123456789012.dkr.ecr.us-west-2.amazonaws.com/ado-agent:v1.2.3-buildtest-.* --push " "$LOG_FILE"
  grep -q "docker pull --platform linux/amd64 123456789012.dkr.ecr.us-west-2.amazonaws.com/ado-agent:v1.2.3-buildtest-" "$LOG_FILE"
  grep -q "container-structure-test test --image 123456789012.dkr.ecr.us-west-2.amazonaws.com/ado-agent:v1.2.3-buildtest-.* --platform linux/amd64" "$LOG_FILE"
  grep -q "docker buildx imagetools create -t 123456789012.dkr.ecr.us-west-2.amazonaws.com/ado-agent:v1.2.3 123456789012.dkr.ecr.us-west-2.amazonaws.com/ado-agent:v1.2.3-buildtest-" "$LOG_FILE"
  grep -q "aws ecr batch-delete-image .* --image-ids imageTag=v1.2.3-buildtest-" "$LOG_FILE"
}
